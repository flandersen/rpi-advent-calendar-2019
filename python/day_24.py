...
try:
    while True:   
        if GPIO.input(t1) == 1:
            za(1)
            p = subprocess.Popen(["omxplayer", "lied_1.mp3"])
            time.sleep(4)
            za(0)
            p.wait()
        if GPIO.input(t2) == 1:
            za(2)
            p = subprocess.Popen(["omxplayer", "lied2.mp3"])
            time.sleep(4)
            za(0)
            p.wait()
        if GPIO.input(t3) == 1:
            za(3)
            p = subprocess.Popen(["omxplayer", "lied3.mp3"])
            time.sleep(4)
            za(0)
            p.wait()
        if GPIO.input(t4) == 1:
            za(4)
            p = subprocess.Popen(["omxplayer", "lied4.mp3"])
            time.sleep(4)
            za(0)
            p.wait()
